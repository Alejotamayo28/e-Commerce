...
export const queryInsertWalletRecord = async (customerId: number, wallet: Wallet, session: PoolClient):
  Promise<Wallet> => {
    const { rows: [walletResponse] } = await session.query<Wallet>(
      `INSERT INTO "Wallet" (customer_id, balance, currency, status)
   VALUES ($1, $2, $3, $4) RETURNING *`,
      [customerId, wallet.balance, wallet.currency, wallet.status])
  return walletResponse
}
export const queryGetWalletRecord = async (customerId: number, session: PoolClient):
  Promise<Wallet> => {
  const { rows: [walletResponse] } = await session.query<Wallet>(
...
export const queryUpdateWalletRecord = async (customerId: number, wallet: Partial<Wallet>, session: PoolClient):
  Promise<Wallet> => {
  const { rows: [walletResponse] }: QueryResult = await session.query<Wallet>(
...
export const UpdateWalletFields = async (customerId: number, wallet: Wallet): Promise<partialWallet> => {
