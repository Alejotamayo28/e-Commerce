...
export const getSellerCountryAndWalletBalance = async (
  productId: number,
  clientId: number,
) => {
...
export const calculateCountryTaxes = (
  user: JwtPayload,
  sellerCountry: string,
): number => {
  let tax = 0;
...
export const createPurchaseRecord = async (
  ...
  const response: QueryResult<Purchase> = await onTransaction(
    async (client) => {
      return await client.query<Purchase>(
...
export const getSellerCountry = async (
  productId: number,
  client: PoolClient,
): Promise<string> => {
