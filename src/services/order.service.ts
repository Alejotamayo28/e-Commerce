...
import {
  ...
  getProductById,
  getSellerCountryAndWalletBalance,
  hasSufficientBalance,
  processPurchase,
} from "../utils/service/order.Utils";

export class OrderService {
  constructor(
    private req: RequestExt,
  ...
  public async createPurchase(product: PurchaseProduct): Promise<Response> {
    ...
    try {
      ...
      const { sellerCountry, walletBalance } =
        await getSellerCountryAndWalletBalance(product.productId, user.id);
